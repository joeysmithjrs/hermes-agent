...
- No versioned workflow registry/catalog (no `catalog.py`). [Re-confirmed by audit 2026-07-30: `ls workflow/store/` has only `__init__.py`, `checkpoint.py`, `fs.py`, `index.py`.]
- No named agent prompt/directive templates; `dsl.py`'s `expr.template(*args, **kw)` raises `NotImplementedError` (`workflow/dsl.py` lines 50-52, nested under the `expr` namespace — there is no top-level `dsl.template`).
- No `debate` node kind in `ir.py` kind enum. [AUDIT CORRECTION: the `NODE_KINDS` tuple lives at `ir.py` lines 37-46, not "line 108" as previously stated. Line 108 is the `REDUCE_TYPES` comment. `NODE_KINDS = ("agent", "script", "fanout", "join", "gate", "map", "cron_trigger", "webhook_trigger")` — `debate` is absent.]
- No `supervisor` kind in `ir.py` kind enum (same `NODE_KINDS` tuple, `ir.py` lines 37-46).
- No backward-route mechanism inside the acyclic graph; only manual chain/schedule form (`run --from-run`, `workflow chain`). `watch` DOES already support `--timeout` (`cli.py` line 84, honored in `_run_watch` around line 391) — the gap is strictly the cycle/loop-back mechanism, not watch timeouts.
- Kanban projection is only a stub/project feature not yet implemented as live projection adapter. [Re-confirmed: `workflow/runtime/kanban.py` `project_run()` returns `{"projected": False, ...}` when `workflow.kanban_projection` is unset — an honest no-op, not a fake success, per its own docstring rationale about `kanban_create` being a dispatchable-work-item API, not a passive card API.]
...
- `prompt`: literal or parametric (same `expr.render` pattern as `expr.py` line 75; AUDIT CORRECTION: previously cited as line 76, which is the docstring line, not the `def render(...)` signature)
- `tags`: list
...
**CONFLICT + RESOLUTION (audit finding, 2026-07-30):** the word "template" is already a live, distinct concept in this codebase — `expr.py`'s `{{ node_id.output.field }}` interpolation syntax (design §2.9), which `verify.py`'s `_check_template_refs` (line 463) compile-time-checks against known node ids (error code `TEMPLATE`). The proposed `workflow/templates/registry.py` is a *different* thing: a library of named, reusable prompt bodies. Shipping both under the word "template" invites confusion (e.g. a user asking "does my prompt's `{{ }}` reference resolve against the templates/ registry or against node outputs?"). Resolution: keep `expr.py`'s interpolation as "template" (already named that in verify.py's `TEMPLATE` error code — renaming it is a bigger footprint than this feature needs) and name the new reusable-prompt-body feature something distinct, e.g. `workflow/prompts/` + `PromptSpec` / `prompt_library.py`, reserving `spec.prompt: {library: <name>, params: {...}}` as the YAML key instead of `{template: <name>, ...}`. This is a naming change only; the mechanism described above (pure loader, no model calls, populates `spec.prompt` pre-`build_child_agent`) is unaffected.

---

=== 5. Workspace mechanism (persistent artifact directory) — NEW, user-requested 2026-07-30; UPDATED 2026-07-30 (persistent named workspace + cross-run persistence + seed context)
The user (Joe) identified a gap: debate/supervisor agents need a shared persistent directory of artifacts (e.g. a living audit document, intermediate protocol-state files) separate from audit artifacts (read-only `store/fs.py`) and separate from manual prompt-context passing.
The user (Joe) refined the design:
- Named, persistent workspace directories (not only per-run): user pins workflow to named workspace (e.g. workspace: "desk-autonomy").
- Structure: top-level persistent folder (workflows/workspaces/<workspace_name>/) with persistent top-level files/dirs (seed context, living audit doc, protocol state, reference artifacts) shared across runs; plus sub-dirs per run (workflows/workspaces/<workspace_name>/runs/<run_id>/) for per-run isolation.
- Agents in persistent workspace review previous runs (read persistent top-level + previous runs/<other_run_id>/ subdirs).
- Cross-run seeding: important things persist at workspace top level (seed.md, protocol_state.yaml); each run references/updates — allows passing context across runs without relying only on chain CLI or prompt-only passing.

VERIFICATION STATUS (honest — this sandbox cannot access `/opt/hermes-agent/workflow/store/fs.py` or `/home/hermes/research/hermes-multi-agent-workflow` directly; labels below indicate source):
- `store/fs.py` behavior (audit artifacts only, read-only after run): CONFIRMED BY PRIOR SESSION (line 22-27 references; `store/` contents verified in prior `claude-pro` session that merged PR #7). NOT re-verified in this sandbox.
- `workspace_kind=dir` mechanism reference (`tonbistudio` dispatcher): UNVERIFIED IN THIS SANDBOX (file `/home/hermes/research/hermes-multi-agent-workflow` inaccessible; mechanism quoted from prior session reference at line 174 of this spec). If unavailable at feature-build time, must be verified before relying on it.
- `workflows/workspaces/` directory exists: NOT FOUND in this sandbox (expected; feature not yet built).

Design (added to spec; audit-only):
- NEW directory layer: `workflows/workspaces/<run_id>/` — separate from `store/fs.py` artifacts (audit) and `workflows/catalog/` (registered recipes). Workspace is ACTIVE (read+write) during run; artifacts remain audit-only.
- Workspace isolation: per-run workspace by default (`workspaces/<run_id>/`). Cross-node workspace sharing within same run only (debate participants, supervisor + advisor). No cross-run workspace sharing (security; avoids hidden persistent injection).
- Agent reference mechanism: workspace path exposed to agent via `LiveWorker` context (not hidden injection). YAML node param: `workspace_dir: "workspaces/<run_id>"` (optional; default to per-run workspace). Agents reference files inside workspace via file-path references in prompts/templates (not a new core tool — uses existing file/tool mechanism); or via a new `prompt_library` parameter `workspace_file: <filename>` that the renderer loads (the renderer reads from the workspace directory at run time, not prompt-cache time, so prompt-caching remains byte-stable for life of conversation — workspace file content changes do NOT corrupt the cached prefix).
- Audit/durability: workspace files saved in audit artifacts under `node_run_id/workspace/` (same naming convention as debate round artifacts). Workspace survives `resume` (workspace dir referenced by `run_id`; resume loads same workspace). `chain` can optionally pass workspace path via `--workspace-from-run` (new CLI option proposal; deferred to CLI layer, not IR core). Workspace reproducibility: workspace files included in any `workdir` snapshot; user can rebuild workspace from saved artifacts for replay.
- Conflict with existing design: workspace does NOT modify `store/fs.py` (audit artifacts untouched); workspace is not a new core model tool (uses file/tool mechanism or prompt renderer); workspace file changes do NOT break prompt caching (workspace content read at agent-run time, not at system-prompt construction time). Workspace reference via YAML param aligns with footprint ladder (CLI/package surface, not core).
- Security / `side_effects`: workspace writes by agent nodes inherit `side_effects: external` rules (same as Phase 2 — gate applies; checkpoint before blocking preserved). Workspace directory must have file-system isolation (same profile; no cross-profile workspace sharing — aligns with per-profile isolation rule in `cron/jobs.py`).

Prior art (verified reference only; mechanism details unverified in this sandbox):
- Tonbi-style dispatcher (`/home/hermes/research/hermes-multi-agent-workflow`): `workspace_kind=dir` indicates persistent workspace per agent/task (directory-based, not only artifact files). Confirmed reference exists at line 174; exact mechanism not re-verified here.
- Hermes native: `delegate_task` (sub-agent spawning), `kanban_decompose` (LLM decomposes triage -> child graph), `kanban_db/store` (persistent cards), `kanban_swarm` — none currently expose a shared workspace dir for agent interaction; this design fills that gap.

Recommendation: workspace mechanism is architecturally sound; must be implemented as NEW directory layer (`workflows/workspaces/`) separate from `store/` artifacts; must verify tonbistudio mechanism before relying on it for design details; must preserve prompt-caching safety (workspace content not embedded in system prompt prefix). Ready for feature build once naming fix (`prompt_library`) + workspace design are adopted.
...
- `protocol`: one of:
  ...
  - `judge_escalate`: after max_rounds or divergence, a `judge` agent (with `judge_model`/`judge_provider` override or parent default) gets the full round history and picks a converged/next-state option. This is close to the "supervisor asks an advisor" model but with bounded rounds + full audit history.
...
**REUSE FINDING (audit, 2026-07-30):** `runtime/scripts.py`'s existing `majority()` reducer (lines 120-161) already implements exactly the `vote` protocol described above — canonicalized-value voting, highest count wins, tie-break = first appearance (lowest branch/round order), with an explicit `tie: true` flag in its output. Debate's `vote` protocol should call the existing `majority` reducer directly rather than adding a new `vote_majority` reducer that duplicates it; only `judge_converge` (the `judge_escalate` path) is genuinely new and needs adding to `runtime/scripts.py`'s frozen bootstrap registration block (`scripts.py` lines 210-217 — reducers can only be added at that one bootstrap site since `register()` raises once `_FROZEN` is set, by design, to close off runtime registry pollution).
...
=== 6. Backward route (loop-back / cycle mechanism) — CLEAN design, not a graph cycle
...
## 7. Hand-off (this is the file to load)
...
This file serves as the durable hand-off artifact for the Phase 3 extensions (persistent registry, prompt templates, debate, supervisor, backward route via chain, workspace mechanism) — all architected cleanly without violating acyclic v1 core design.
