...
def _check_node(
    ...
    if n.kind == "join":
        ...
        if n.reduce and n.reduce.get("type") not in (None, "concat", "top_k", "first_k", "majority"):
            err("REDUCE", n.id, f"unknown reduce type '{n.reduce.get('type')}'")
        elif n.reduce and n.reduce.get("type") in ("first_k", "majority"):
            # Phase 1 implements only concat + top_k; first_k/majority silently
            # fall through to concat in the driver (review P2). Reject them now
            # rather than returning wrong results; richer reducers are Phase 3.
            err("REDUCE", n.id, f"reduce type '{n.reduce.get('type')}' not implemented in Phase 1 (use concat/top_k)")
